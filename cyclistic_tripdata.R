...
# Create a data frame to keep the fields needed for analysis.
...
# Run some calculations to get a better sense of the data layout.
...
# Explore the characteristics of each kind of user
...
type_of_bike_used <- cyclistic_tripdata_time %>% filter(!is.na(rideable_type)) %>%  group_by(member_casual, rideable_type) %>% 
...
# Create a data frame with the longitutde and latitude data to use it in Tableau and make a map visualization.
